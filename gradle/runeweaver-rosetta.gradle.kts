...
val configuredRosettaPath = providers.gradleProperty("rosetta_jar")
    .map { file(it) }
    .orNull
val rosettaVersion = providers.gradleProperty("deps.rosetta").get()

val rosettaJarPath = configuredRosettaPath?.let { configured ->
    if (configured.isDirectory) {
        listOf(
            configured.resolve("build/release/Rosetta-$rosettaVersion-${project.name}.jar"),
            configured.resolve("libs/rosetta/${project.name}/Rosetta-$rosettaVersion-${project.name}.jar"),
...
val publishCompanion = providers.gradleProperty("publish_runeweaver_rosetta")
...
val buildCompanion = tasks.register("buildRuneweaverRosetta") {
    group = "build"
    description = "Builds the optional Runeweaver/Rosetta companion artifact for this target."
}
...
if (companionEnabled) {
    ...
    val companion = sourceSets.maybeCreate("runeweaverRosetta")
    ...
    val companionJar = tasks.register<Jar>("runeweaverRosettaJar") {
    ...
    extensions.configure<PublishingExtension> {
        publications.create<MavenPublication>("runeweaverRosetta") {
            ...
            pom {
                name = "Runeweaver Rosetta (${project.name})"
                description = "Server-owned datapack asset synchronization for Runeweaver and Rosetta."
            url = "https://github.com/Rasa-Novum/runeweaver"
                ...
                withXml {
                    ...
                    dependency("com.rasanovum.runeweaver", "runeweaver-${project.name}", project.version.toString())
                    dependency(
    ...
    buildCompanion.configure {
        doFirst {
            throw GradleException("Pass -Ppublish_runeweaver_rosetta=true or -Prosetta_jar=<target Rosetta jar> to build the Runeweaver Rosetta companion")
        }
    }
}
