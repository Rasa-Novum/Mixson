...
import static net.rasanovum.runeweaver.util.RuneweaverUtil.deserializeFile;
import static net.rasanovum.runeweaver.util.RuneweaverUtil.overlappingIndices;
...
public class EventContext<T> {
    ...
    private final RuneweaverRuntime<?> runtime;
    ...
    protected EventContext(T file, Index index, EventEntry<T> entry, RuneweaverRuntime<?> runtime, Map.Entry<Index, Resource> resourceEntry, boolean markedForDeletion) {
    ...
    public RuneweaverEvent<T> getEvent() {
        return entry.event();
    }
    ...
    public List<Mutable<T>> captureFiles(Index id) {
        if(overlappingIndices(capturedFiles.keySet(), id)) {
            runtime.error(new RuneweaverException("cannot capture same file twice"), this.getEvent(), resourceEntry.getKey().id());
            return List.of();
        }
        ...
        for(Map.Entry<Index, List<Mutable<T>>> captureEntry : capturedFiles.entrySet()) {
            ...
            for(Mutable<T> resource : captureEntry.getValue()) {
                ...
                if(resourceFile == null) {
                    runtime.error(new RuneweaverException("captured file with index {} cannot be null", captureEntry.getKey()), getEvent(), captureEntry.getKey().id());
                }
